...
payrollSchema.statics.getTotalData = async function(fromDate, toDate, group, id){
    ...
    const pipeline = [
        ...
        {$group:{
            _id: null,
            total_payroll:{$push: '$$ROOT'},
            total_grosspay: {$sum: '$salaries.gross_salary'},
            total_days: {$sum: '$attendance.whalf_days'},
            total_netpay: {$sum: '$salaries.net_salary'},
        }},
        {$addFields: {
            total_payroll:{$size: '$total_payroll'}
        }},
        {$group:{
            _id: null,
            total_payroll:{$first: '$total_payroll'},
    ...
    let result = await employees.aggregate(pipeline)
